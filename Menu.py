        ...
        if opcion == 1:
            ...
            with open("bloques\\"+nombreA) as csvfile:
                ...
                if listaDoble.head is None:
                    ...
                    nodoNuevoC = node()
                    now = datetime.now()
                    nodoNuevoC.INDEX = contador
                    nodoNuevoC.PREVIOUSHASH = listaDoble.getUltimo().HASH
                    nodoNuevoC.TIMESTAMP = now.strftime("%d-%m-%Y::%H:%M:&S")
                    nodoNuevoC.CLASS = datos[0]
                    nodoNuevoC.DATA = datos[1]
                    nodoNuevoC.HASH = encrypt_string(str(nodoNuevoC.INDEX) + nodoNuevoC.TIMESTAMP + nodoNuevoC.CLASS + nodoNuevoC.DATA +str(nodoNuevoC.PREVIOUSHASH)  )
                    listaDoble.add(nodoNuevoC)
                    contador+=1
                    print("Ya tiene info")
            os.system('pause')
